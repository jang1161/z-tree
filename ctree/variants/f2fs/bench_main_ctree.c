...
/* ── F2FS variant: CNS via sparse file instead of raw device ── */
#define CTREE_CNS_MOUNT_PATH  "/mnt/cns"
...
static void reset_device(const char *dev_path)
{
    ...
    /* Re-format F2FS on CNS device and remount */
    rc = system("sudo umount " CTREE_CNS_MOUNT_PATH " 2>/dev/null; "
                "sudo mkfs.f2fs -f " CTREE_CNS_DEV_PATH " && "
                "sudo mount " CTREE_CNS_DEV_PATH " " CTREE_CNS_MOUNT_PATH);
    if (rc != 0)
        fprintf(stderr, "WARNING: F2FS format/mount failed (rc=%d)\n", rc);
}
