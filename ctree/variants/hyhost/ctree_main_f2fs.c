...
static void *dynamic_gc_thread(void *arg)
{
    ...
    while (!atomic_load_explicit(&g_gc_stop, memory_order_acquire))
    {
        ...
        /* evict+punch are non-blocking (per-node node_trywrlock); inserts run
         * throughout.  Freeze leaf spill for the cycle so it can't refill CNS
         * faster than evict drains it. */
        if (g_gc_freeze_spill)
            atomic_store_explicit(&g_leaf_cns_frozen, true, memory_order_release);
        if (g_cns_gc_blocking)
            pthread_rwlock_wrlock(&g_insert_pause);   /* stop-the-world */
        struct timespec t0, t1, t2;
        clock_gettime(CLOCK_MONOTONIC, &t0);
        cow_phase_mark(t, is_hwm ? "begin:cns_hwm_evict" : "begin:cns_periodic_evict");
        cow_evict_cns_leaves(t);
        cow_phase_mark(t, is_hwm ? "end:cns_hwm_evict" : "end:cns_periodic_evict");
        ...
        if (g_gc_freeze_spill)
            atomic_store_explicit(&g_leaf_cns_frozen, false, memory_order_release);
        double evict_s = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;
        double punch_s = (t2.tv_sec - t1.tv_sec) + (t2.tv_nsec - t1.tv_nsec) / 1e9;
        fprintf(stderr,
                "[ctree_dynamic] CNS GC done (%s): evict=%.3fs  punch=%.3fs  "
                "total=%.3fs  freeze=%d\n",
                reason, evict_s, punch_s, evict_s + punch_s, g_gc_freeze_spill);
        elapsed_periodic = 0;
    }
    return NULL;
}
...
retry_flush:
    ...
    if (!cns_path)
    {
        ...
        if (cns_bitmap_test(t, pg->node_id)) {
            ...
            cns_dirty_set(t, pg->node_id);  /* genuine CNS->ZNS: slot now reclaimable */
        }
    }
    else
    {
