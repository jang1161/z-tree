 ...
 *                  + CNS fallback on zone write-lock contention
 ...
 gcc -O2 -g -Wall -Wextra -std=c11 -pthread \
      ...
      ctree/variants/parent_rewrite/ctree_main.c \
      ctree/bench_main_ctree.c \
      -Ictree/variants/parent_rewrite -Ictree \
      -o build/ctree_parrew -lzbd -lnvme -lpthread
 *
 * ────────────────────────────────────────────────────────────────────────── */
...
retry_flush:
    ...
    else
    {
        ...
        slot_id = pg->node_id;
        /* Set bit 63 so CNS cache tags never collide with ZNS page numbers */
...
static void do_single_insert(ztree_t *t, int64_t key, const char *value)
{
    for (uint32_t retry = 0;; retry++)
    {
        ...
        /* Propagate to parents on split OR zone migration (paper-faithful).
         * When a child moves to a different zone, parent's child_zone_id
         * must be updated to match, as per ZTree paper §3.2 UPDATE step 6. */
