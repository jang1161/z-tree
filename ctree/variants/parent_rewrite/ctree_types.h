/*
 * ctree_types.h  –  Shared type definitions for CTree
 *
 * CoW B+-Tree for ZNS NVMe with CNS (Conventional Namespace) fallback:
 ...
 *   4. CNS Fallback: on zone write-lock contention, spill to /dev/nvme3n1
 */
 ...
 * Active-zone budget (device limit=14): IZ(3)+Hot(7)+Cold(2)+Meta(1)=13.
...
/* ── CNS (Conventional Namespace) fallback ─────────────────────────────────
 * When a zone write-lock is contended, CTree spills the page to a conventional
 * block device (/dev/nvme3n1) instead of blocking.  Pages on CNS are tracked
 * with a special zone_id sentinel so NLT/read path can route reads correctly.
 */
