...
retry_flush:
    ...
    if (!sticky_ok)
    {
        ...
        if (!sticky_ok)
        {
            ...
            {
                cns_path = true;
                /* Atomic bump-allocate a slot on the shared CNS device. */
...
cow_tree *cow_open(const char *path)
{
    ...
    t->cns_wp = calloc(1, sizeof(*t->cns_wp));
    if (t->cns_wp)
        atomic_store_explicit(t->cns_wp, 0, memory_order_relaxed);
    t->cns_wp_owned = 1;
