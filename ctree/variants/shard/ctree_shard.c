/*
 * ctree_shard.c  –  Tree-level sharding wrapper for CTree
 *
 * 13 independent tree structures (root, node latches, NLT, node-ID space)
 * sharing the same ZNS zone pool + CNS device.  Combines CTree's trylock +
 * CNS fallback with sharding to reduce both zone-lock and node-latch contention.
 *
 * Key routing: key % CTREE_NUM_SHARDS.
 *
 gcc -O2 -g -Wall -Wextra -std=c11 -pthread \
      ctree/ctree_nlt.c ctree/ctree_zone.c \
      ctree/variants/shard/ctree_main.c \
      ctree/variants/shard/ctree_shard.c ctree/variants/shard/bench_shard.c \
      -Ictree/variants/shard -Ictree \
      -o build/ctree_shard -lzbd -lnvme -lpthread
 */
...
#include "ctree_shard.h"

/* Clone shard-specific state from the primary.
 * Shares: fd, direct_fd, cns_fd, info, zones, zone_wp_bytes, zone_full,
...
static ztree_t *shard_clone(ztree_t *base, uint32_t shard_id)
{
    ...
    /* ── Shared CNS state ──────────────────────────────────────── */
    t->cns_fd = base->cns_fd;           /* shared fd */
    t->cns_wp = base->cns_wp;           /* shared bump allocator */
    t->cns_wp_owned = 0;                /* clone does NOT free cns_wp */
...
static void shard_close_clone(ztree_t *t)
{
     ...
     * Do NOT free shared resources (fd, cns_fd, zones, zone_write_locks,
...
shard_tree_t *shard_open(const char *dev_path)
{
    ...
    for (int i = 1; i < CTREE_NUM_SHARDS; i++) {
...
{
    uint32_t shard = (uint32_t)((uint64_t)key % CTREE_NUM_SHARDS);
...
void shard_close(shard_tree_t *st)
{
    ...
    for (int i = CTREE_NUM_SHARDS - 1; i >= 1; i--)
