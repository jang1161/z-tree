#!/bin/bash
# bench_sequential.sh — 5-way sequential-insert comparison
#
# Runs:
#   1. ztree              (build/ztree)
...
#
# Each variant inserts $KEYS keys in ascending order (BENCH_SEQUENTIAL=1
# disables the Fisher-Yates shuffle in the bench main).  ZNS is reset and
# the CNS file is wiped between every run so each variant starts fresh.
#
# Usage:
#   sudo bash scripts/bench_sequential.sh [keys] [threads]
...
OUTDIR="$REPO_ROOT/logs/sequential/${KEYS}_T${THREADS}_${TS}"
mkdir -p "$OUTDIR"
...
echo "=== Sequential insert ($KEYS keys, $THREADS threads) ==="
echo "Device: $ZNS"
echo "Output: $OUTDIR"
echo ""
...
for v in "${VARIANTS[@]}"; do
    ...
    sudo BENCH_SEQUENTIAL=1 $EXTRA_ENV "$REPO_ROOT/$BIN" \
