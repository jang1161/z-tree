...
/*
 * flush_page_immediate  –  write one CoW page to the ZNS device.
 *
 *   prev_zone   The zone_id where the node currently lives, or
 *               ZTREE_INVALID_ZONE_ID for new nodes (splits / first insert).
 *
 *   avoid_zone  When allocating via round-robin (splits), skip this zone_id
 *               so that a new sibling always lands in a different zone from
 *               its existing counterpart.  Pass ZTREE_INVALID_ZONE_ID to
 *               impose no constraint.
 *
 * Zone selection follows the paper (§3.2):
 *
 *   1. Stickiness (Normal Update): if prev_zone is valid and not yet sealed
 *      (< 95% used), append to prev_zone.  The ZoneID stays constant so
 *      parent nodes need NOT be rewritten.
 *
 *   2. Dynamic_Allocation (Split / Sealed Zone): call zone_alloc_* to pick
 *      the next zone in the group via Round-robin.  If the chosen zone equals
 *      avoid_zone, skip it – enforcing the paper's "sibling in a different
 *      zone" rule.
 */
...
retry_flush:
    ...
    /* ── Phase 2: Per-zone group-commit batch ────────────────────────── */
    zbatch_entry_t my_entry;
    atomic_store_explicit(&my_entry.done, 0, memory_order_relaxed);
    my_entry.pg = pg;
    my_entry.slot_id = 0;
    my_entry.pn = ZTREE_INVALID_PGN;

    zbatch_queue_t *bq = &t->zone_batch[target_zone];
    pthread_mutex_lock(&bq->mu);

    /* If queue is full, wait for the current flusher to drain. */
    while (bq->count >= ZTREE_BATCH_MAX)
        pthread_cond_wait(&bq->cv, &bq->mu);

    bq->entries[bq->count] = &my_entry;
    bq->count++;

    if (!bq->flushing)
    {
        /* This thread becomes the flusher for this zone's batch queue. */
        bq->flushing = true;
        ...
        while (bq->count > 0)
        {
            ...
            zbatch_entry_t *batch[ZTREE_BATCH_MAX];
            for (uint32_t i = 0; i < N; i++)
                batch[i] = bq->entries[i];

            /* Shift remaining entries down. */
            for (uint32_t i = N; i < bq->count; i++)
                bq->entries[i - N] = bq->entries[i];
            bq->count -= N;

            pthread_mutex_unlock(&bq->mu);

            /* Acquire zone write lock. */
            ...
            uint64_t zwl_hold_start = lock_t1;

            /* Check if zone became full while we waited. */
            ...
            /* Check zone capacity for N pages. */
            uint64_t zone_start = t->zones[target_zone].start;
            uint64_t zone_cap   = t->zones[target_zone].capacity;
            uint64_t zone_end   = zone_start + zone_cap;
            uint64_t wp = atomic_load_explicit(&t->zone_wp_bytes[target_zone],
                                               memory_order_relaxed);

            if (wp + (uint64_t)N * ZTREE_PAGE_SIZE > zone_end)
            {
                /* Zone cannot fit all N pages. Mark zone full and retry all. */
                record_zwl_hold(t, target_zone, monotonic_ns() - zwl_hold_start);
                pthread_mutex_unlock(&t->zone_write_locks[target_zone]);
            ...
            /* Reserve WP for N pages at once. */
            atomic_store_explicit(&t->zone_wp_bytes[target_zone],
                                  wp + (uint64_t)N * ZTREE_PAGE_SIZE,
                                  memory_order_relaxed);

            /* Stamp each page's zone_id and slot_id, build the I/O buffer. */
            int wfd = (t->direct_fd >= 0) ? t->direct_fd : t->fd;
            _Alignas(4096) char batch_buf[ZTREE_BATCH_MAX * ZTREE_PAGE_SIZE];

            for (uint32_t i = 0; i < N; i++)
            {
                uint64_t page_wp = wp + (uint64_t)i * ZTREE_PAGE_SIZE;
                uint32_t sid = (uint32_t)((page_wp - zone_start) / ZTREE_PAGE_SIZE);
                ztree_pagenum_t pn = (ztree_pagenum_t)(page_wp / ZTREE_PAGE_SIZE);

                batch[i]->pg->zone_id = target_zone;
                batch[i]->pg->slot_id = sid;
                batch[i]->slot_id = sid;
                batch[i]->pn = pn;

                if (t->direct_fd >= 0)
                    memcpy(batch_buf + (size_t)i * ZTREE_PAGE_SIZE,
                           batch[i]->pg, ZTREE_PAGE_SIZE);
            }

            /* Single pwrite for all N pages. */
            const void *wbuf;
            size_t write_len = (size_t)N * ZTREE_PAGE_SIZE;
            ...
            {
                /* Non-direct fd with N>1: need contiguous buffer. */
                for (uint32_t i = 0; i < N; i++)
                    memcpy(batch_buf + (size_t)i * ZTREE_PAGE_SIZE,
                           batch[i]->pg, ZTREE_PAGE_SIZE);
                wbuf = batch_buf;
            ...
            ssize_t pwr = pwrite(wfd, wbuf, write_len, (off_t)wp);
            if (pwr != (ssize_t)write_len)
            {
                int e = errno;
                if (e == EOVERFLOW)
                {
                    /* Too many active zones — undo WP reservation. */
                    atomic_fetch_sub_explicit(&t->zone_wp_bytes[target_zone],
                                             (uint64_t)N * ZTREE_PAGE_SIZE,
                                             memory_order_relaxed);
                    record_zwl_hold(t, target_zone, monotonic_ns() - zwl_hold_start);
                    pthread_mutex_unlock(&t->zone_write_locks[target_zone]);
                    usleep(500);
                ...
                fprintf(stderr,
                        "flush_page_immediate: batch pwrite at 0x%llx len=%zu "
                        "ret=%zd errno=%d (%s) target_zone=%u N=%u\n",
                        (unsigned long long)wp, write_len, pwr, e, strerror(e),
                        target_zone, N);
                exit(EXIT_FAILURE);
            }

            /* Successful batch pwrite — record hold time. */
            record_zwl_hold(t, target_zone, monotonic_ns() - zwl_hold_start);
            pthread_mutex_unlock(&t->zone_write_locks[target_zone]);

            /* Seal check: use the new WP after all N pages. */
            uint64_t new_wp = wp + (uint64_t)N * ZTREE_PAGE_SIZE;
            if (new_wp >= zone_end)
            {
                atomic_store_explicit(&t->zone_full[target_zone], 1, memory_order_release);
                zone_seal_and_replace(&t->za, target_zone);
            ...
            {
                uint64_t zone_bytes_used = new_wp - zone_start;
                uint64_t seal_threshold = (zone_cap * 95ULL) / 100ULL;
        ...
        /* No more entries in queue — relinquish flusher role. */
        bq->flushing = false;
        pthread_cond_broadcast(&bq->cv);
        pthread_mutex_unlock(&bq->mu);
    }
    ...
    {
        /* This thread is a waiter — block until the flusher processes our entry. */
        while (atomic_load_explicit(&my_entry.done, memory_order_acquire) == 0)
            pthread_cond_wait(&bq->cv, &bq->mu);
        pthread_mutex_unlock(&bq->mu);
    }

    /* Check if we need to retry (zone was full). */
    if (atomic_load_explicit(&my_entry.done, memory_order_acquire) == -1)
    {
        sticky_ok = false;
        goto retry_flush;
    }

    /* ── Phase 3: Post-write per-thread work ─────────────────────────── */
    uint32_t slot_id = my_entry.slot_id;
    ztree_pagenum_t pn = my_entry.pn;
                                         ...
                                         uint32_t root_slot)
{
    ...
    uint64_t actual_seq_before = atomic_load_explicit(&t->volatile_sb.seq_no, memory_order_acquire);
    ...
    if (!cas_success)
    {
        // fprintf(stderr, "[DEBUG CAS FAIL] expected_seq=%lu (even=%lu), actual_before=%lu, actual_after_fail=%lu, root_nid=%u, root_zone=%u, root_slot=%u\n",
        //         expected_seq_even, expected_seq_even, actual_seq_before, expected,
        //         (unsigned)root_nid, root_zone, root_slot);
        return 0;
    }

    // fprintf(stderr, "[DEBUG CAS OK] seq: %lu -> %lu (succeeded)\n", expected_seq_even, expected_seq_even + 2);
    ...
    for (uint32_t retry = 0;; retry++)
    {
        ...
        if (prop.split)
        {
            ...
            need_root_publish = 1;
                ...
                // fprintf(stderr,
                //     "[ZTREE ROOT SPLIT] new_root_id=%u root_zone=%u root_slot=%u seq_snapshot=%lu\n",
...
cow_tree *cow_open(const char *path)
{
    ...
    /* Per-zone group-commit batch queues. */
    t->zone_batch = calloc(t->info.nr_zones, sizeof(zbatch_queue_t));
    if (!t->zone_batch)
    {
        perror("cow_open zone_batch");
        for (uint32_t z2 = 0; z2 < t->info.nr_zones; z2++)
            pthread_mutex_destroy(&t->zone_write_locks[z2]);
        free(t->zone_write_locks);
        for (size_t i = 0; i < ZTREE_NODE_LATCH_BUCKETS; i++)
            pthread_rwlock_destroy(&t->node_latches[i]);
        free(t->node_latches);
    ...
    for (uint32_t z = 0; z < t->info.nr_zones; z++)
    {
        pthread_mutex_init(&t->zone_batch[z].mu, NULL);
        pthread_cond_init(&t->zone_batch[z].cv, NULL);
        t->zone_batch[z].count = 0;
        t->zone_batch[z].flushing = false;
    }

    /*
     * Zone layout (paper-aligned):
     *   Zones 0-1:   RLayer (meta ping-pong)
     *   Zones 2-17:  ILayer pool  → initial IZGroup = zones 2-5 (4 zones)
     *   Zones 18+:   LLayer pool  → initial LZGroup = 8 zones (hot=6, cold=2)
     *                80% of LLayer pool → hot; 20% → cold
     */
...
#undef _US
    ...
    if (t->zone_batch)
    {
        for (uint32_t z = 0; z < t->info.nr_zones; z++)
        {
            pthread_mutex_destroy(&t->zone_batch[z].mu);
            pthread_cond_destroy(&t->zone_batch[z].cv);
        }
        free(t->zone_batch);
    }
...
ztree_record *ztree_find(ztree_t *t, int64_t key)
{
    ...
    while (!pg.is_leaf)
    {
        ...
        /* Resolve via NLT (NLT may have a fresher slot than the page field) */
        nlt_location_t query = {
            .zone_id = child_zone,
            .node_id = child_nid,
            .slot_id = ZTREE_INVALID_SLOT_ID,
        ...
        {
            /* Fallback: use zone_id from parent and scan the zone's latest slot.
             * This should only happen if the NLT hasn't been populated yet,
             * which is resolved by the lazy-population in load_page_by_pn.   */
