/*
 * ztree_main.h  –  ZTree handle (io_uring variant)
 *
 * Replaces per-zone mutex + sync pwrite with:
 *   - atomic WP reservation (no zone lock during I/O)
 *   - io_uring async write + mq-deadline scheduler for zone ordering
 */
...
#define ZTREE_MAX_THREADS            64
#define ZTREE_RING_DEPTH              8
...
struct ztree_s {
    ...
    /* Concurrency */
    pthread_rwlock_t *node_latches;
    pthread_mutex_t  *zone_write_locks; /* kept for zone_alloc seal/finish */

    /* io_uring: per-thread rings */
    struct io_uring   rings[ZTREE_MAX_THREADS];
    _Atomic(uint32_t) ring_next;         /* round-robin assignment */
    int               ring_count;
