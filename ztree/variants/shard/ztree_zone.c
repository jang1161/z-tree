                              ...
                              const char *layer_name)
{
    for (;;) {
        ...
        {
            /* Fallback expansion (shard-aware): grow only, don't finish.
             *
             * With multiple sharded trees sharing the same zone pool,
             * many zones can fill simultaneously.  If we finish ALL
             * outgoing zones here, the subsequent zone_seal_and_replace
             * calls (one per sealed zone) see "already FULL" via the
             * dedup check and skip their grow — losing N-1 growths.
             *
             * Instead, we only grow the count by 1 (the CAS above
             * already did that).  zone_seal_and_replace handles each
             * zone's finish + grow individually, ensuring 1:1 mapping
             * between seals and growths.  The outer for(;;) loop
             * retries the scan and finds the new zone. */
            (void)za; /* finish is delegated to zone_seal_and_replace */
        }
    }
}
...
void zone_seal_and_replace(zone_alloc_t *za, uint32_t zone_id)
{
    ...
    /* 1. Finish the sealed zone.
     *
     * No verify-and-retry: the EOVERFLOW retry in flush_page_immediate
     * handles the case where the device hasn't fully committed the
     * OPEN→FULL transition before a new zone is opened.  This makes
     * zone_seal_and_replace fast (no usleep loop), which is critical
     * for sharded workloads where many independent trees fill zones
     * concurrently. */
