 ...
 * ztree_main.c  –  ZTree core: cache, NLT, zone alloc, single-pass CoW insert
 ...
 gcc -O2 -g -Wall -Wextra -std=c11 -pthread \
      ztree/ztree_nlt.c ztree/ztree_zone.c ztree/ztree_main.c \
...
static void do_single_insert(ztree_t *t, int64_t key, const char *value)
{
    for (uint32_t retry = 0;; retry++)
    {
        ...
        /* Propagate to parents only when split or child zone migration occurred. */
        ...
        {
            /* Root was rewritten (CoW append produced a new slot).  Publish
             * the new (zone, slot) so ztree_find — which loads root via raw
             * (zone, slot), not NLT — sees the latest content.  Same-zone
             * rewrites still allocate a fresh slot, so a spurious no-op
             * publish is filtered by the equality check below. */
 ...
 *
 * Concurrency (b1, mirroring INSERT):
 *   Pass 1 — Optimistic: read-crab descent + leaf wrlock upgrade.  If the
 *            leaf has >= MIN keys after delete (or is root), CoW append
 *            it and propagate any zone change up via bottom-up ancestor
 *            wrlocks (same as insert's child_zone_id propagation).
 *   Pass 2 — Pessimistic: write-latch all ancestors top-down so sibling
 *            merges + cascades can be applied atomically.  Used only when
 *            Pass 1 detects the leaf will underflow.
 *
 * NLT note: when sibling is merged into cur, we do NOT call nlt_remove on
 * the sibling.  The sibling's NLT entry stays pointing at its last on-
 * disk slot, which is still valid for readers holding stale parent
 * snapshots.  Without GC, leaving the entry preserves snapshot
 * consistency and avoids re-introducing the stale-NLT race we just
 * eliminated.  This deviates from paper Algorithm 4 line 11.
 * ═══════════════════════════════════════════════════════════════════════════ */
...
/* Release wrlocks on path[0..n-1], coalescing duplicates by latch bucket. */
...
/* Bottom-up zone-change propagation for SIMPLE delete (no merge).
 * Acquires/releases ancestor wrlocks one level at a time, mirroring the
 * insert propagate loop.  Returns 0 on success, -1 on race retry. */
...
/* Pass 1: read-crab descent → leaf wrlock → simple delete (no merge).
 * Returns DEL_OK / DEL_NOT_FOUND / DEL_RACE / DEL_NEEDS_MERGE. */
                                 ...
                                 int *out_root_changed)
{
    ...
    while (1) {
        ...
        if (f->page.is_leaf) {
            /* Upgrade rdlock → wrlock at the leaf. */
            node_unlock(t, cur_id);
            node_wrlock(t, cur_id);
            if (!load_latest_node(t, cur_zone, cur_id,
                                  &f->zone_id, &f->slot_id, &f->page)) {
        ...
        cur_zone = (cidx == RIGHTMOST_IDX) ? f->page.ptr_zone_id
                                            : f->page.internal[cidx].child_zone_id;
        cur_id = next_id;
        path[depth + 1].cidx_from_parent = cidx;  /* set for next frame */
        depth++;
    }

    /* At leaf with wrlock held. */
    ...
    /* Apply simple delete: remove key, CoW append, propagate zone change up. */
    for (uint32_t i = (uint32_t)found_idx; i + 1 < leaf->num_keys; i++)
        leaf->leaf[i] = leaf->leaf[i + 1];
    leaf->num_keys = new_count;

    /* Empty root leaf → publish INVALID root (tree empty). */
    ...
    /* Root was rewritten iff prop.left_id == root_nid.  Always publish
     * the new (zone, slot) — same-zone CoW still allocates a fresh slot,
     * and a no-op equality check in cow_delete filters spurious publish. */
...
/* Pass 2: pessimistic write-latch crab + merge cascade + root collapse. */
                                  ...
                                  int *out_root_changed)
{
    ...
    while (1) {
        ...
        /* Check if next's latch bucket already held by some path frame. */
        int already_held = 0;
    ...
    /* At leaf with all path[0..depth-1] wrlocked. */
    delete_path_frame *leaff = &path[depth - 1];
    ztree_page *leaf = &leaff->page;
    int found_idx = -1;
    for (uint32_t i = 0; i < leaf->num_keys; i++) {
    ...
    /* Remove key from leaf in-place. */
    for (uint32_t i = (uint32_t)found_idx; i + 1 < leaf->num_keys; i++)
        leaf->leaf[i] = leaf->leaf[i + 1];
    leaf->num_keys--;

    /* Empty root leaf → tree empty. */
    ...
    memset(&prop, 0, sizeof prop);
    int level = depth - 1;
    int merge_done_at_level = -1;  /* level (index in path) of the highest node that has been flushed during the cascade */
    ...
    /* Cascade loop: at each level, if cur is underfilled, try sibling merge.
     * If merge reduces parent's keys, cascade up.  Otherwise stop. */
    while (level > 0) {
        ...
        if (cur->num_keys >= min_keys) {
            /* Cur is no longer underfilled (e.g., merge filled it back).
             * Just CoW append cur and propagate up. */
            break;
        }

        /* Pick sibling: prefer right (cidx+1 or RIGHTMOST), else left (cidx-1). */
        ...
        if (cidx == RIGHTMOST_IDX) {
            if (par->num_keys == 0) break;  /* parent has only this child */
            ...
            sib_cidx = par->num_keys;  /* sentinel: sibling is par->ptr */
        ...
        /* Acquire sibling wrlock if not already held by path. */
        ...
        if (cur->is_leaf) {
            ...
            merged_keys = cur_keys + 1 + sib_keys;  /* +1 separator from parent */
            max_keys = ZTREE_INTERNAL_ORDER - 1;
        }

        if (merged_keys > max_keys) {
            /* Can't merge.  Leave cur underfilled and stop cascade.
             * Paper Algorithm 4 explicitly accepts this case. */
            if (!sib_already_held) node_unlock(t, sib_id);
            break;
        }

        /* ── Merge cur and sibling.  Surviving node is cur (we keep cur's
         *    node_id and rewrite parent to drop the sibling). ─────────── */
        if (cur->is_leaf) {
            if (sib_is_right) {
            ...
            /* Internal merge: left ++ separator ++ right.  Separator key
             * comes from parent (the one BETWEEN the two children). */
            uint64_t separator;
            if (sib_is_right) {
                separator = par->internal[cidx].key;
            } else {
                separator = par->internal[sib_cidx].key;  /* sib_cidx = cidx-1 */
            }
        ...
        /* ── Flush the modified parent (separator + sibling-pointer removed,
         *    cur's child_zone updated to merged location).  This is the
         *    second CoW append of the merge step. ───────────────────────── */
        ...
        if (level - 1 == 0) {
            /* Parent is root, has keys, and was just CoW-appended above.
             * prop.left_id == root_nid → publish below. */
            break;
        }
        ...
        break;  /* parent OK, stop cascade */
    }

    /* Now CoW append every level from merge_done_at_level (or leaf if no
     * merge happened) up to root, applying any zone-change propagation.
     * If merge_done_at_level == 0 with collapse signal, the root is
     * already set in prop and we skip the root-level append. */

    /* If no merge happened at all (cur was either not underfilled, or
     * couldn't merge with its sibling), CoW append the leaf as-is.
     * The leaf is the only node flushed in this branch. */
    if (!root_collapsed && merge_done_at_level < 0) {
        ...
        /* Propagate-up will start at leaf's parent (depth-2) iff the leaf
         * migrated zones; otherwise the loop guard short-circuits. */
        merge_done_at_level = depth - 1;
    }

    /* Walk from (merge_done_at_level - 1) up to 0, propagating zone changes.
     * The cascade itself already flushed levels merge_done_at_level..depth-1;
     * we only need to update ancestors that route to whatever was last
     * flushed (prop.left_id at prop.left_zone). */
    if (!root_collapsed) {
    ...
    /* Decide what (if anything) to publish to volatile_sb:
     *   - root_collapsed: tree height shrunk by one; new root pulled from
     *     old root's ptr_* via NLT.
     *   - prop.left_id == root_nid: root was rewritten in place (same node_id,
     *     new slot via CoW append).
     *   - otherwise: only non-root nodes were flushed; volatile_sb stays.
     */
    if (root_collapsed) {
...
int cow_delete(cow_tree *t, int64_t key)
{
    for (uint32_t retry = 0;; retry++) {
        ...
        if (res == DEL_NEEDS_MERGE) {
            ...
            if (res == DEL_NOT_FOUND) return 0;
            if (res == DEL_RACE) continue;
            /* DEL_OK falls through */
        }

        /* res == DEL_OK at this point. */
        if (root_changed &&
